...
namespace SteamBotExample.Steam2FA
{
	public class AuthenticatorLinker
	{
		...
		public LinkResult AddAuthenticator()
		{
			...
			string response = SteamWebAuth.MobileLoginRequest(APIEndpoints.STEAMAPI_BASE + "/ITwoFactorService/AddAuthenticator/v0001", "POST", postData);
		...
		public FinalizeResult FinalizeAddAuthenticator(string smsCode)
		{
			...
			while (tries <= 30)
			{
				...
				string response = SteamWebAuth.MobileLoginRequest(APIEndpoints.STEAMAPI_BASE + "/ITwoFactorService/FinalizeAddAuthenticator/v0001", "POST", postData);
		...
		{
			string response = SteamWebAuth.Request(APIEndpoints.COMMUNITY_BASE + "/steamguard/phoneajax?op=add_phone_number&arg=" + WebUtility.UrlEncode(PhoneNumber), "GET", null, _cookies);
		...
		private bool _hasPhoneAttached()
		{
			...
			string response = SteamWebAuth.MobileLoginRequest(APIEndpoints.COMMUNITY_BASE + "/steamguard/phoneajax", "GET", postData, _cookies);
			if (response == null) return false;
