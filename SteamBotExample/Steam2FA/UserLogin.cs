	...
	public class UserLogin
	{
		...
		public LoginResult DoLogin()
		{
			...
			if (cookies.Count == 0)
			{
				...
				SteamWebAuth.MobileLoginRequest("https://steamcommunity.com/login?oauth_client_id=DE45CD61&oauth_scope=read_profile%20write_profile%20read_client%20write_client", "GET", null, cookies, headers);
			}

			postData.Add("username", this.Username);
			response = SteamWebAuth.MobileLoginRequest(APIEndpoints.COMMUNITY_BASE + "/login/getrsakey", "POST", postData, cookies);
			if (response == null) return LoginResult.GeneralFailure;
			RSAResponse rsaResponse = null;
			...
			using (var rsaEncryptor = new RSACryptoServiceProvider())
			{
				...
				rsaParameters.Exponent = Common.HexStringToByteArray(rsaResponse.Exponent);
				rsaParameters.Modulus = Common.HexStringToByteArray(rsaResponse.Modulus);
			...
			postData.Add("donotcache", Common.GetSystemUnixTime().ToString());

			response = SteamWebAuth.MobileLoginRequest(APIEndpoints.COMMUNITY_BASE + "/login/dologin", "POST", postData, cookies);
			if (response == null) return LoginResult.GeneralFailure;
